...
class XReadsCollection():
    ...
    def run_collect_phased_reads_for_one_chrm_v2(self, record):
        ...
        for pos in m_sites[site_chrm]:
            ...
            sf_out_file_hap1 = working_folder + "{0}_{1}_{2}.fa".format(site_chrm, pos, HAP1)
            sf_out_file_hap2 = working_folder + "{0}_{1}_{2}.fa".format(site_chrm, pos, HAP2)
            sf_out_file_hap_unknown = working_folder + "{0}_{1}_{2}.fa".format(site_chrm, pos, HAP_UNKNOWN)
            sf_out_file_discord = working_folder + "{0}_{1}_{2}.fa".format(site_chrm, pos, HAP_DISCORD)
            ####for calling internal mutations

            sf_out_hap1_slct = working_folder + "{0}_{1}_{2}.fa".format(site_chrm, pos, HAP1_SLCT)
            sf_out_hap2_slct = working_folder + "{0}_{1}_{2}.fa".format(site_chrm, pos, HAP2_SLCT)
            sf_out_hap_unknown_slct = working_folder + "{0}_{1}_{2}.fa".format(site_chrm, pos, HAP_UNKNOWN_SLCT)
            sf_out_hap_all_slct = working_folder + "{0}_{1}_{2}.fa".format(site_chrm, pos, ALL_HAP_SLCT)
            sf_out_hap_disc_slct= working_folder + "{0}_{1}_{2}.fa".format(site_chrm, pos, DISC_HAP_SLCT)
            ...
            for sbarcode in set_all:
                ...
                # print chrm #############################################################################################
                iflank=i_extend
                l_picked_algnmts = self.filter_out_unique_non_related_algnmts(l_alignmts, UNIQUE_MAP_CUTOFF,
