...
3. pre-processed repeat library used by xTea (The same library is used for both short and long reads)  
	...
	Or directly download through `wget https://github.com/parklab/xTea/blob/master/rep_lib_annotation.tar.gz`. 
...
2. **Run the pipeline**
	...
	+ Run on a slurm cluster or a single node (by default `xtea` assume the reference genome is **GRCh38** and **hg38**, for `hg19` or `GRCh37`, please use `xtea_hg19`)
		+ Only with Illumina data
			```
			xtea -i sample_id.txt -b illumina_bam_list.txt -x null -p ./path_work_folder/ -o submit_jobs.sh -n 8 -l /home/rep_lib_annotation/ -r /home/reference/genome.fa -g /home/gene_annotation_file.gff3 --xtea /home/ec2-user/xtea/ --flklen 3000 -f 5907 -y 7 			```

		+ Only with 10X data
			```
			xtea -i sample_id.txt -b null -x 10X_bam_list.txt -p ./path_work_folder/ -o submit_jobs.sh -n 8 -l /home/ec2-user/rep_lib_annotation/ -r /home/ec2-user/reference/genome.fa -g /home/gene_annotation_file.gff3  --xtea /home/ec2-user/xtea/ --flklen 3000 -y 7 -f 5907 			```
		
		+ Working with hybrid data of 10X and Illumina 
			```
			xtea -i sample_id.txt -b illumina_bam_list.txt -x 10X_bam_list.txt -p ./path_work_folder/ -o submit_jobs.sh -n 8 -l /home/ec2-user/rep_lib_annotation/ -r /home/ec2-user/reference/genome.fa -g /home/gene_annotation_file.gff3 --xtea /home/ec2-user/xtea/ --flklen 3000 -y 7 -f 5907
			```
		+ Working with case-ctrl mode
			```
			xtea --case_ctrl --tumor -i sample_id.txt -b case_ctrl_bam_list.txt -p ./path_work_folder/ -o submit_jobs.sh -n 8 -l /home/ec2-user/rep_lib_annotation/ -r /home/ec2-user/reference/genome.fa -g /home/gene_annotation_file.gff3 --xtea /home/ec2-user/xtea/ --flklen 3000 -y 7 -f 5907
			```
		...
		+ Parameters:
			...
			Required:
				...
				-y: type of repeats will work on (1-L1, 2-Alu, 4-SVA, 8-HERV; sum all selected as one value. For example, if want to check L1 and SVA only, then set `-y 5`);
			...
			Cutoffs will be automatically set based on the read depth and the purity if it is a tumor sample; parameters have been thoroughly tuned based on the test on benchmark data and also large cohort analysis. We will add an extra manual for advanced users to set the major parameters soon.
			```
