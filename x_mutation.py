...
class XMutation():
    def __init__(self, s_working_folder):
        ...
        sf_mutation_folder=self.working_folder+MUTATION_FOLDER
    ...
    def call_snp_in_parallel(self, l_records, n_jobs, sf_merged_vcf):
        ...
        s_mutation_flder = self.working_folder + MUTATION_FOLDER + "/"
        cmd="{0} merge ".format(BCFTOOLS_PATH)
        for rcd in l_records:
            site_chrm = rcd[0]
            pos = rcd[1]
            sf_vcf = s_mutation_flder + "{0}_{1}_{2}.vcf.gz".format(site_chrm, pos, ALL_HAP_SLCT)
            cmd+=(sf_vcf+" ")
    ...
    def align_reads_to_cns_one_site(self, record):
        ...
        sf_cns=record[3]
        s_reads_folder=self.working_folder + READS_FOLDER + "/"
        bwa_align = BWAlign(BWA_PATH, BWA_REALIGN_CUTOFF, n_cores)
        sfa_hap1_slct = s_reads_folder + "{0}_{1}_{2}.fa".format(site_chrm, pos, HAP1_SLCT)
        sfa_hap2_slct = s_reads_folder + "{0}_{1}_{2}.fa".format(site_chrm, pos, HAP2_SLCT)
        sfa_hap_unknown_slct = s_reads_folder + "{0}_{1}_{2}.fa".format(site_chrm, pos, HAP_UNKNOWN_SLCT)
        sfa_hap_all_slct = s_reads_folder + "{0}_{1}_{2}.fa".format(site_chrm, pos, ALL_HAP_SLCT)

        s_mutation_flder = self.working_folder + MUTATION_FOLDER + "/"
        ssam_hap1_tmp = s_mutation_flder + "{0}_{1}_{2}.tmp.bam".format(site_chrm, pos, HAP1_SLCT)
        ssam_hap2_tmp = s_mutation_flder + "{0}_{1}_{2}.tmp.bam".format(site_chrm, pos, HAP2_SLCT)
        ssam_hap_unknown_tmp = s_mutation_flder + "{0}_{1}_{2}.tmp.bam".format(site_chrm, pos, HAP_UNKNOWN_SLCT)
        ssam_hap_all_tmp = s_mutation_flder + "{0}_{1}_{2}.tmp.bam".format(site_chrm, pos, ALL_HAP_SLCT)

        # align reads to the consensus
        bwa_align.realign_reads_to_bam(SAMTOOLS_PATH, sf_cns, sfa_hap1_slct, ssam_hap1_tmp)
        bwa_align.realign_reads_to_bam(SAMTOOLS_PATH, sf_cns, sfa_hap2_slct, ssam_hap2_tmp)
        bwa_align.realign_reads_to_bam(SAMTOOLS_PATH, sf_cns, sfa_hap_unknown_slct, ssam_hap_unknown_tmp)
        bwa_align.realign_reads_to_bam(SAMTOOLS_PATH, sf_cns, sfa_hap_all_slct, ssam_hap_all_tmp)

        ssam_hap1_slct = s_mutation_flder + "{0}_{1}_{2}.sorted.bam".format(site_chrm, pos, HAP1_SLCT)
        ssam_hap2_slct = s_mutation_flder + "{0}_{1}_{2}.sorted.bam".format(site_chrm, pos, HAP2_SLCT)
        ssam_hap_unknown_slct = s_mutation_flder + "{0}_{1}_{2}.sorted.bam".format(site_chrm, pos, HAP_UNKNOWN_SLCT)
        ssam_hap_all_slct = s_mutation_flder + "{0}_{1}_{2}.sorted.bam".format(site_chrm, pos, ALL_HAP_SLCT)
    ...
    def call_snp_from_reads_alignment(self, sf_algnmt, sf_ref, sf_vcf):
        ...
        cmd="{0} mpileup -IOu -f {1} {2} | {3} call -vmO z -o {4}".format(BCFTOOLS_PATH, sf_ref, sf_algnmt,
                                                                     BCFTOOLS_PATH, sf_vcf_tmp)
        Popen(cmd, shell=True, stdout=PIPE).communicate()
    ...
    def _index_vcf_gz(self, sf_input):
        cmd = "{0} -f -p vcf {1}".format(TABIX_PATH, sf_input)
        Popen(cmd, shell=True, stdout=PIPE).communicate()
