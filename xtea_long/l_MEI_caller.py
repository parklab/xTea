...
def unwrap_self_filter_by_TE_kmer(arg, **kwarg):
    return L_MEI_Caller.filter_by_TE_kmer_one_site(*arg, **kwarg)
...
class L_MEI_Caller():
    def __init__(self, s_wfolder, n_jobs, sf_ref):
        ...
        self.kmerlib = KmerLib()

    ####
    def set_rep_lib_path_kmer_cnt(self, i_type, sf_rep_folder):#
        lrc = LRepClassification(self.wfolder, self.n_jobs)
        b_hg19=False
        lrc.set_rep_configuration(i_type, sf_rep_folder, b_hg19)
        l_sf_cns=lrc.get_sf_cns_kmer_cnt()#
        return l_sf_cns
...
####
    ...
    # this version align the flank regions to contig sequence
    # Note: in some cases the called out breakpoint is not accuracy, which may cause problem
    def call_MEIs_for_sites_with_rep_kmer_filtering(self, sf_bam_list, sf_sites, sf_ref, l_extd_len, sf_out_fa,
                                                    sf_out_sites, l_rep_cns, n_cutoff, n_min_non_polyA_kmer, n_min_polyA_kmer,
                                                    b_complex, b_asm_cmd=False,
                                                    sf_script_prefix="", b_call_seq=True, b_skip_exist=False):#
        '''
        :param sf_bam_list: a list of long reads bam/cram files
        :param sf_sites: candidate site list
        :param sf_ref: reference genome
        ...
        print("Begin to construct the TE kmer library!\n")
        self.kmerlib.construct_kmer_lib2(l_rep_cns)#construct the kmer library from consensus sequence
        print("The TE kmer library is constructed/loaded!\n")
...
####
    ...
    def filter_by_TE_kmer_in_parallel(self, l_sites, n_min_non_polyA_kmer_hit=1, n_min_polyA_kmer_hit=1, n_cutoff=1):
        ...
        for (sf_bam, ins_chrm, ins_pos, wfolder, b_pass) in l_rslts:
            if b_pass==True:
                l_passed.append((sf_bam, ins_chrm, ins_pos, wfolder))
        return l_passed

    def filter_by_TE_kmer_one_site(self, record):#
        sf_bam = record[0]
        ins_chrm = record[1]
        ins_pos = record[2]
        wfolder = record[3]
...
####
    ...
    ####
    def _chk_ins_by_TE_kmer_lib(self, record):####
        ins_chrm = record[1]
        ins_pos = record[2]
        wfolder = record[3]
        ...
        with open(sf_fa) as fin_fa:#
            for line in fin_fa:#
                if len(line)>0 and line[0]==">":#
                    continue
                s_fa=line.rstrip()
                b_seq_hit=self.kmerlib.is_seq_contain_TE_kmer_rc_non_rc(s_fa, n_min_non_polyA_kmer_hit, n_min_polyA_hit)
                if b_seq_hit==True:
                    n_hit_cnt+=1
                    if n_hit_cnt>=n_cutoff: ######
                        ...
                        break
                b_rc=True
                b_seq_hit_rc=self.kmerlib.is_seq_contain_TE_kmer_rc_non_rc(s_fa, n_min_non_polyA_kmer_hit, n_min_polyA_hit, b_rc)
                if b_seq_hit_rc==True:
                    n_hit_cnt_rc+=1
...
####
    ...
    def _clean_intermediate_files_in_parallel(self, l_prefix, l_flanks):
        pool = Pool(self.n_jobs)
        pool.map(unwrap_self_remove_intermediate_files, list(zip([self] * len(l_prefix), l_prefix)), 1)
        pool.close()
        pool.join()

        pool = Pool(self.n_jobs)
        pool.map(unwrap_self_remove_intermediate_flank_files, list(zip([self] * len(l_flanks), l_flanks)), 1)
        pool.close()
        pool.join()
####
####
