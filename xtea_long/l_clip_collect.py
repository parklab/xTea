...
class LRDClipReadInfo():
    ...
    def collect_seqs_for_sites_in_parallel(self, l_sites):
        ...
        pool.map(unwrap_self_collect_clip_parts_lrd, list(zip([self] * len(l_sites), l_sites)), 1)
        pool.close()
        pool.join()
    ...
    def collect_seqs_cover_regions_in_parallel(self, l_sites):#
        ...
        pool = Pool(self.n_jobs)
        pool.map(unwrap_self_collect_reads_cover_region_lrd, list(zip([self] * len(l_sites), l_sites)), 1)
        pool.close()
        pool.join()
####
