...
#grnt calling steps
def gnrt_calling_command(iclip_c, iclip_rp, idisc_c, iflt_clip, iflt_disc, ncores, iflk_len, iflag):
    ...
    sf_collect="python ${{XTEA_PATH}}\"x_TEA_main.py\" -E --nb 500 -b ${{BAM1}} -d ${{BARCODE_BAM}} --ref ${{REF}} " \
            "-i ${{PREFIX}}\"candidate_disc_filtered_cns.txt\" -p ${{TMP}} -n {0}\n".format(ncores)
    ...
    sf_mutation="python ${{XTEA_PATH}}\"x_TEA_main.py\" -I -p ${{TMP}} -n {0} " \
                        "-i ${{PREFIX}}\"high_confident_for_mutation.list\" -r ${{L1_CNS}} " \
                        "-o ${{PREFIX}}\"internal_snp.vcf.gz\"\n".format(ncores)
...
if __name__ == '__main__':
    ...
    iflag=options.flag
    s_calling_cmd=gnrt_calling_command(iclip_c, iclip_rp, idisc_c, iflt_clip, iflt_disc, ncores, iflk_len, iflag)
