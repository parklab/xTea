...
def gnrt_lib_config(l_rep_type, sf_folder_rep, sf_ref, sf_folder_xtea, sf_config_prefix):
    if sf_folder_rep[-1] != "/":
        sf_folder_rep += "/"
    if sf_folder_xtea[-1] != "/":
        sf_folder_xtea += "/"
    ...
    for s_rep_type in l_rep_type:
        if s_rep_type=="L1":# for L1
            ...
            write_to_config(sf_anno, sf_ref, sf_copy_with_flank, sf_flank, sf_cns, sf_xtea, s_bl, s_bam1, s_bc_bam,
                            s_tmp, s_tmp_clip, s_tmp_cns, sf_config_HERV)
            ...
            write_to_config(sf_anno, sf_ref, sf_copy_with_flank, sf_flank, sf_cns, sf_xtea, s_bl, s_bam1, s_bc_bam,
                            s_tmp, s_tmp_clip, s_tmp_cns, sf_config_Mitochondrion)
...
def gnrt_running_shell(l_rep_type, sf_bam, s_wfolder, ncores, sf_folder_rep, sf_ref, sf_folder_xtea):
    if s_wfolder[-1] != "/":
        s_wfolder += "/"
    ...
    gnrt_lib_config(l_rep_type, sf_folder_rep, sf_ref, sf_folder_xtea, sf_sample_folder)
...
if __name__ == '__main__':
    ...
    if i_rep_type & 8 != 0:
        l_rep_type.append("HERV")
    if i_rep_type & 16 != 0:
        l_rep_type.append("Mitochondrion")
