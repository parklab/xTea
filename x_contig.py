...
from clip_read import LRClipReadInfo
...
class XTEContig():
    ...
    def align_long_contig(self, sf_ref, sf_contig, n_jobs, sf_algnmt):
        cmd = "{0} -x asm5 --cs -a -t {1} {2} {3} > {4}".format(MINIMAP2, n_jobs, sf_ref, sf_contig, sf_algnmt)
        self.run_cmd(cmd)

    def align_short_contig_minimap2(self, sf_ref, sf_contig, n_jobs, sf_algnmt):
        cmd = "{0} -x sr --cs -a -t {1} {2} {3} > {4}".format(MINIMAP2, n_jobs, sf_ref, sf_contig, sf_algnmt)
        self.run_cmd(cmd)
    ...
    def align_short_contigs_minimap2_v2_sam(self, sf_ref, sf_contig, n_jobs, sf_algnmt):
        cmd = "{0} -k15 -w5 --sr --frag=yes -A2 -B8 -O12,32 -E2,1 -r150 -p.5 -N20 -f10000,50000 -n2 " \
              ...
              "> {4}" \
            .format(MINIMAP2, n_jobs, sf_ref, sf_contig, sf_algnmt)
        self.run_cmd(cmd)
    ...
    def align_short_contig_minimap2_in_bam(self, sf_ref, sf_contig, n_jobs, sf_algnmt):
        cmd = "{0} -x sr --cs -a -t {1} {2} {3} | samtools view -hSb - | " \
              "samtools sort -o {4} -".format(MINIMAP2, n_jobs, sf_ref, sf_contig, sf_algnmt)
        self.run_cmd(cmd)
        cmd = "samtools index {0}".format(sf_algnmt)
        self.run_cmd(cmd)
    ...
    def align_short_contig_bwa(self, sf_ref, sf_contig, n_jobs, sf_algnmt):
        ...
        if os.path.isfile(sf_bwt) == False:
            cmd = "{0} index {1}".format(BWA, sf_ref)
            self.run_cmd(cmd)
        cmd = "{0} mem -t {1} {2} {3} > {4}".format(BWA, n_jobs, sf_ref, sf_contig, sf_algnmt)
        self.run_cmd(cmd)
    ...
    def run_align_flank_to_contig_by_chrm(self, record):
        ...
        with open(sf_sites) as fin_sites:
            for line in fin_sites:
                ...
                sf_asm = working_folder + "{0}/{1}_{2}/contig.fa".format(ASM_FOLDER, chrm, tmp_pos)
                sf_flank = working_folder + '{0}/{1}_{2}_flanks.fa'.format(FLANK_FOLDER, chrm, tmp_pos)
                if os.path.exists(sf_asm) == False or os.path.exists(sf_flank) == False:
                    continue
                if os.stat(sf_asm).st_size == 0:
                    continue
                sf_algnmt_folder = working_folder + MAP_FOLDER
        ...
        with open(sf_sites) as fin_sites:
            for line in fin_sites:
                ...
                if os.path.exists(sf_algnmt_folder) == False:
                    ...
                    self.run_cmd(cmd)
                ###all the reads
                sf_asm_all = working_folder + "{0}/{1}_{2}/{3}/contig.fa".format(ASM_FOLDER, chrm, tmp_pos, ALL_HAP)
                if os.path.exists(sf_asm_all) != False and os.stat(sf_asm_all).st_size != 0:  ####
                    sf_algnmt_all = "{0}/{1}_{2}_{3}.sam".format(sf_algnmt_folder, chrm, tmp_pos, ALL_HAP)
                    self.align_short_contigs_minimap2_v2_sam(sf_asm_all, sf_flank, 1, sf_algnmt_all)
                ###haplotype 1
                sf_asm_hap1 = working_folder + "{0}/{1}_{2}/{3}/contig.fa".format(ASM_FOLDER, chrm, tmp_pos, HAP1)
                if os.path.exists(sf_asm_hap1) != False and os.stat(sf_asm_hap1).st_size != 0:  ####
                    sf_algnmt1 = "{0}/{1}_{2}_{3}.sam".format(sf_algnmt_folder, chrm, tmp_pos, HAP1)
                    self.align_short_contigs_minimap2_v2_sam(sf_asm_hap1, sf_flank, 1, sf_algnmt1)
                ###haplotype 2
                sf_asm_hap2 = working_folder + "{0}/{1}_{2}/{3}/contig.fa".format(ASM_FOLDER, chrm, tmp_pos, HAP2)
                if os.path.exists(sf_asm_hap2) != False and os.stat(sf_asm_hap2).st_size != 0:  ####
                    sf_algnmt2 = "{0}/{1}_{2}_{3}.sam".format(sf_algnmt_folder, chrm, tmp_pos, HAP2)
                    self.align_short_contigs_minimap2_v2_sam(sf_asm_hap2, sf_flank, 1, sf_algnmt2)
                ###unknown haplotype
                sf_asm_hap_unknown = working_folder + "{0}/{1}_{2}/{3}/contig.fa".format(ASM_FOLDER, chrm, tmp_pos,
                                                                                         HAP_UNKNOWN)
                if os.path.exists(sf_asm_hap_unknown) != False and os.stat(sf_asm_hap_unknown).st_size != 0:  ####
                    sf_algnmt_unknown = "{0}/{1}_{2}_{3}.sam".format(sf_algnmt_folder, chrm, tmp_pos, HAP_UNKNOWN)
                    self.align_short_contigs_minimap2_v2_sam(sf_asm_hap_unknown, sf_flank, 1, sf_algnmt_unknown)
                ###discord reads
                sf_asm_hap_discord = working_folder + "{0}/{1}_{2}/{3}/contig.fa".format(ASM_FOLDER, chrm, tmp_pos,
                                                                                         HAP_DISCORD)
                if os.path.exists(sf_asm_hap_discord) != False and os.stat(sf_asm_hap_discord).st_size != 0:  ####
                    sf_algnmt_disc = "{0}/{1}_{2}_{3}.sam".format(sf_algnmt_folder, chrm, tmp_pos, HAP_DISCORD)
    ...
    def run_filter_out_non_TE_from_asm_by_chrm(self, record):
        ...
        with open(sf_sites) as fin_sites, open(sf_chrm_out, "w") as fout_chrm_true_positive:
            for line in fin_sites:
                ...
                sf_algnmt_folder = self.working_folder + MAP_FOLDER
                ...
                try:
                    ...
                    for algnmt in samfile.fetch():  # check each alignment, and find "left" and "right" flanks
                        ...
                        if f_map_ratio < f_map_cutoff:
                            continue
                        if algnmt.query_name == LEFT_FLANK:
                            b_left_mapped = True
                            if algnmt.is_reverse == True:
                                b_left_rc = True
                            i_left_pos = int(mpos)
                            l_ref = algnmt.reference_name
                        elif algnmt.query_name == RIGHT_FLANK:
                            b_right_mapped = True
                            if algnmt.is_reverse == True:
                                b_right_rc = True
    ...
    def filter_out_non_TE_from_asm(self, sf_sites, flank_length, f_map_cutoff, i_slack, sf_kept_sites):
        ...
        with open(sf_sites) as fin_sites:
            for line in fin_sites:
                ...
                m_chrms[chrm] = 1
        for chrm in m_chrms:
            sf_chrm_out = self.working_folder + FILTER_FOLDER + "/"
        ...
        with open(sf_kept_sites, "w") as fout_final:
            for chrm in m_chrms:
                sf_chrm_list = self.working_folder + FILTER_FOLDER + "/" + "{0}.filtered".format(chrm)
    ...
    def is_qualified_TE_insertion_strict_version_v2(self, sf_contig, sf_algnmt, flank_length, f_map_cutoff, i_slack):
        b_qualified = False
        b_left_mapped = False
        b_right_mapped = False
        b_left_rc = False
        ...
        try:
            ...
            for algnmt in samfile.fetch():  # check each alignment, and find "left" and "right" flank
                ...
                if l_cigar[0][0] == 4 and l_cigar[0][
                    1] >= TEI_ON_CNS_CLIP_LENTH:  # the clip part should be long (>99bp)
                    b_l_clip = True
                if l_cigar[-1][0] == 4 and l_cigar[-1][1] >= TEI_ON_CNS_CLIP_LENTH:
                    b_r_clip = True
                mpos = int(algnmt.reference_start)
    ...
    def is_qualified_TE_insertion_loosen_version_v2(self, sf_contig, sf_algnmt, flank_length, f_map_cutoff, i_slack):
        ...
        try:
            ...
            for algnmt in samfile.fetch():  # check each alignment, and find "left" and "right" flank
                ...
                b_r_clip = False  # right clip
                if l_cigar[0][0] == 4 and l_cigar[0][
                    1] >= TEI_ON_CNS_CLIP_LENTH:  # the clip part should be long (>74bp)
                    b_l_clip = True
                if l_cigar[-1][0] == 4 and l_cigar[-1][1] >= TEI_ON_CNS_CLIP_LENTH:  #
                    b_r_clip = True
                mpos = int(algnmt.reference_start)
    ...
    def run_select_TEI_from_algnmt_by_chrm(self, record):
        ...
        sf_TEI_seq_folder = working_folder + TEI_SEQ_FOLDER
        ...
        with open(sf_sites) as fin_sites, open(sf_TEI_seq_chrm, "w") as fout_tei_seqs:
            for line in fin_sites:  ###for each candidate site
                ...
                b_qualifed, seq1, seq2 = self.is_qualified_TE_insertion_strict_version_v2(sf_asm_all, sf_algnmt_all,
                                                                                          ...
                                                                                          i_slack)
                ###Note, here use SEPERATOR to seperate chrm and pos, in case chrm contains "_"
                s_site = "{0}{1}{2}".format(select_chrm, SEPERATOR, tmp_pos)
                ...
                sf_algnmt_hap1 = "{0}/{1}_{2}_{3}.sam".format(sf_algnmt_folder, chrm, tmp_pos, HAP1)
                sf_asm_hap1 = working_folder + "{0}/{1}_{2}/{3}/contig.fa".format(ASM_FOLDER, chrm, tmp_pos, HAP1)
                ...
                sf_algnmt_hap2 = "{0}/{1}_{2}_{3}.sam".format(sf_algnmt_folder, chrm, tmp_pos, HAP2)
                sf_asm_hap2 = working_folder + "{0}/{1}_{2}/{3}/contig.fa".format(ASM_FOLDER, chrm, tmp_pos, HAP2)
                ...
                sf_algnmt_hap_unknown = "{0}/{1}_{2}_{3}.sam".format(sf_algnmt_folder, chrm, tmp_pos, HAP_UNKNOWN)
                sf_asm_hap_unknown = working_folder + "{0}/{1}_{2}/{3}/contig.fa".format(ASM_FOLDER, chrm, tmp_pos,
                                                                                         HAP_UNKNOWN)
                ...
                sf_algnmt_hap_disc = "{0}/{1}_{2}_{3}.sam".format(sf_algnmt_folder, chrm, tmp_pos, HAP_DISCORD)
                sf_asm_hap_disc = working_folder + "{0}/{1}_{2}/{3}/contig.fa".format(ASM_FOLDER, chrm, tmp_pos,
                                                                                      HAP_DISCORD)
        ...
        with open(sf_chrm_out, "w") as fout_chrm_true_positive:
            ...
            for s_pos in m_final:
                s_pos_fields = s_pos.split(SEPERATOR)
    ...
    def validate_TEI_from_phased_asm_algnmt(self, sf_sites, flank_length, f_map_cutoff, i_slack, sf_repeat_copies,
        ...
        sf_TEI_seq_folder = self.working_folder + TEI_SEQ_FOLDER  ####create the folder
        if os.path.exists(sf_TEI_seq_folder) == False:
    ...
    ####################################################################################################################
    ####merge the contigs of one type (ALL_HAP, HAP1, HAP2, HAP_UNKNOWN, HAP_DISCORD) to a single file
    ...
    def call_MEIs_from_masked_algnmt(self, sf_msk_algmt, sf_ref, sf_out_list, sf_seqs, sf_2clip):
        ...
        for algnmt in iter_algnmts:
            ...
            site_fields = query_name.split(SEPERATOR)
            ...
            if len(l_cigar) == 1 and l_cigar[0][0] == 0:  ##fully mapped
                if len(query_seq) < MIN_CONTIG_CLIP_LENTH:
                    continue
                ...
                if len(mapped_seq) < MIN_CONTIG_CLIP_LENTH:
                    continue
            ...
            for site_chrm in m_selected:
                for site_pos in m_selected[site_chrm]:
                    ...
                    for s_seq in m_seqs[site_chrm][site_pos]:
                        shead = ">{0}{1}{2}{3}{4}\n".format(site_chrm, SEPERATOR, site_pos, SEPERATOR, icnt)
                        icnt += 1
                    ...
                    if site_chrm in m_clip_seqs and site_pos in m_clip_seqs[site_chrm]:
                        ...
                        for (s_seq, s_dir) in m_clip_seqs[site_chrm][site_pos]:
                            ####
                            shead = ">{0}{1}{2}{3}{4}{5}{6}\n".format(site_chrm, SEPERATOR, site_pos, SEPERATOR, s_dir,
                                                                      SEPERATOR, icnt)
                            icnt += 1
    ...
    def call_MEIs_from_group_contigs(self, sf_ref, sf_cns, s_working_folder, stype, sf_in, sf_out, sf_seqs):
        s_algnmt_folder = s_working_folder + MAP_FOLDER + "/"
        sf_hap_bam = s_algnmt_folder + stype + ".sorted.bam"
        ####
        s_seq_folder = s_working_folder + TEI_SEQ_FOLDER + "/"
        self.creat_new_folder_no_exist(s_seq_folder)

        lrclip = LRClipReadInfo(sf_hap_bam, self.n_jobs, sf_ref, s_seq_folder)
        islack = 200
    ...
    def call_MEIs_from_all_group_contigs(self, sf_sites, sf_ref, sf_cns, s_working_folder, sf_out_sites, sf_out_seqs):
        ...
        ####Hap1
        sf_out_hap1 = s_seq_folder + HAP1 + ".candidate.list"  ###candidate list called from hap1
        sf_seqs_hap1 = s_seq_folder + HAP1 + ".seqs.fa"
        self.call_MEIs_from_group_contigs(sf_ref, sf_cns, s_working_folder, HAP1, sf_sites, sf_out_hap1, sf_seqs_hap1)
        self.call_final_list_from_hap(sf_out_hap1, "1/0", m_final_list)

        ####Hap2
        sf_out_hap2 = s_seq_folder + HAP2 + ".candidate.list"  ###candidate list called from hap2
        sf_seqs_hap2 = s_seq_folder + HAP2 + ".seqs.fa"
        self.call_MEIs_from_group_contigs(sf_ref, sf_cns, s_working_folder, HAP2, sf_sites, sf_out_hap2, sf_seqs_hap2)
        self.call_final_list_from_hap(sf_out_hap2, "0/1", m_final_list)

        ####Hap_all
        sf_out_hap_all = s_seq_folder + ALL_HAP + ".candidate.list"  ###candidate list called from all hap
        sf_seqs_hap_all = s_seq_folder + ALL_HAP + ".seqs.fa"
        self.call_MEIs_from_group_contigs(sf_ref, sf_cns, s_working_folder, ALL_HAP, sf_sites, sf_out_hap_all,
                                          sf_seqs_hap_all)
        self.call_final_list_from_hap(sf_out_hap_all, "1/1", m_final_list)

        ####Hap_unkown
        sf_out_hap_uk = s_seq_folder + HAP_UNKNOWN + ".candidate.list"  ###candidate list called from unkown hap
        sf_seqs_hap_uk = s_seq_folder + HAP_UNKNOWN + ".seqs.fa"
        self.call_MEIs_from_group_contigs(sf_ref, sf_cns, s_working_folder, HAP_UNKNOWN, sf_sites, sf_out_hap_uk,
                                          sf_seqs_hap_uk)
        self.call_final_list_from_hap(sf_out_hap_uk, "un", m_final_list)

        ####Hap_discord
        sf_out_hap_disc = s_seq_folder + HAP_DISCORD + ".candidate.list"  ###candidate list called from disc hap
        sf_seqs_hap_disc = s_seq_folder + HAP_DISCORD + ".seqs.fa"
        self.call_MEIs_from_group_contigs(sf_ref, sf_cns, s_working_folder, HAP_DISCORD, sf_sites, sf_out_hap_disc,
                                          sf_seqs_hap_disc)
        ...
        cmd = "cat {0} {1} {2} {3} {4} > {5}".format(sf_seqs_hap1, sf_seqs_hap2, sf_seqs_hap_all, sf_seqs_hap_uk,
                                                     sf_seqs_hap_disc, sf_out_seqs)
        Popen(cmd, shell=True, stdout=PIPE).communicate()
    ...
    def merge_contigs_by_type(self, sf_sites, stype, working_folder, sf_out):
        with open(sf_out, "w") as fout_fa, open(sf_sites) as fin_sites:
            for line in fin_sites:
                ...
                chrm = fields[0]
                tmp_pos = fields[1]
                sf_asm = working_folder + "{0}/{1}_{2}/{3}/contig.fa".format(ASM_FOLDER, chrm, tmp_pos, stype)
    ...
    def align_one_group_contigs(self, sf_sites, stype, working_folder, sf_ref):
        s_asm_folder = working_folder + ASM_FOLDER + "/"
        self.creat_new_folder_no_exist(s_asm_folder)
        s_algnmt_folder = working_folder + MAP_FOLDER + "/"
    ...
    def align_asm_contigs_to_reference(self, sf_sites, sf_ref, working_folder):
        ...
        self.align_one_group_contigs(sf_sites, HAP1, working_folder, sf_ref)
        # merge and align for hap2
        self.align_one_group_contigs(sf_sites, HAP2, working_folder, sf_ref)
        # merge and align for all-hap
        self.align_one_group_contigs(sf_sites, ALL_HAP, working_folder, sf_ref)
        # merge and align for unknown
        self.align_one_group_contigs(sf_sites, HAP_UNKNOWN, working_folder, sf_ref)
        # merge and align for discord
        self.align_one_group_contigs(sf_sites, HAP_DISCORD, working_folder, sf_ref)
