...
##Revised based on the old version to create a wrapper for de novo insertion calling from trio data
...
def gnrt_calling_command(iclip_c, iclip_rp, idisc_c, iflt_clip, iflt_disc, ncores, iflk_len, min_tei_len, iflag,
    ...
    sclip_step = "python ${{XTEA_PATH}}\"x_TEA_main_denovo_cloud.py\" -C -i ${{BAM_LIST}} --lc {0} --rc {1} --cr {2}  " \
    ...
    if b_SVA is True:
        sclip_step = "python ${{XTEA_PATH}}\"x_TEA_main_denovo_cloud.py\" -C --sva -i ${{BAM_LIST}} --lc {0} --rc {1} --cr {2}  " \
                     "-r ${{L1_COPY_WITH_FLANK}}  -a ${{ANNOTATION}} --cns ${{L1_CNS}} --ref ${{REF}} -p ${{TMP}} " \
                     "-o ${{PREFIX}}\"candidate_list_from_clip.txt\"  -n {3} --cp {4} {5} {6} {7} {8} {9}\n".format(iclip_c, iclip_c,
                                                                                                    iclip_rp, ncores,
                                                                                                    s_cfolder, s_purity, s_user, s_clean, s_tumor, s_resume)
    sdisc_step = "python ${{XTEA_PATH}}\"x_TEA_main_denovo_cloud.py\"  -D -i ${{PREFIX}}\"candidate_list_from_clip.txt\" --nd {0} " \
    ...
    if b_SVA is True:
        sdisc_step = "python ${{XTEA_PATH}}\"x_TEA_main_denovo_cloud.py\"  -D --sva -i ${{PREFIX}}\"candidate_list_from_clip.txt\" --nd {0} " \
                     "--ref ${{REF}} -a ${{ANNOTATION}} -b ${{BAM_LIST}} -p ${{TMP}} " \
                     "-o ${{PREFIX}}\"candidate_list_from_disc.txt\" -n {1} {2} {3} {4} {5}\n".format(idisc_c, ncores, s_purity, s_user, s_tumor, s_resume)
####
    s_filter = "python ${{XTEA_PATH}}\"x_TEA_main_denovo_cloud.py\" -N --cr {0} --nd {1} -b ${{BAM_LIST}} -p ${{TMP_CNS}} " \
    ...
    sf_trsdct = "python ${{XTEA_PATH}}\"x_TEA_main_denovo_cloud.py\" --transduction --cr {0} --nd {1} -b ${{BAM_LIST}} -p ${{TMP_TNSD}} " \
...
####
    ####
    sf_post_filter = "python ${{XTEA_PATH}}\"x_TEA_main_denovo_cloud.py\" --postF --rtype {0} -p ${{TMP_CNS}} " \
                     ...
                     "-o ${{PREFIX}}\"candidate_disc_filtered_cns_post_filtering.txt\"\n".format(i_rep_type, ncores,
                                                                                                 s_tumor, s_hard_cut)
####
    sf_post_filter_hc = "python ${{XTEA_PATH}}\"x_TEA_main_denovo_cloud.py\" --postF --rtype {0} -p ${{TMP_CNS}} -n {1} " \
...
def gnrt_calling_command_denovo_from_trio(iflt_clip, iflt_disc, ncores, sf_ctr_list, iflk_len,
    ...
    s_cmd = ""
    s_case_control = "python ${{XTEA_PATH}}\"x_TEA_main_denovo_cloud.py\" --denovo -p ${{TMP_CNS}} -b {0} " \
    ...
    ####BamSnap
    s_bamsnap = "python ${{XTEA_PATH}}\"x_TEA_main_denovo_cloud.py\" --igv --bamsnap --single_sample -p ${{PREFIX}}\"tmp/igv\" -b {0} " \
...
def gnrt_running_shell(s_family_id, sf_case_bam, sf_paternal_bam, sf_maternal_bam, l_rep_type, s_wfolder,
    ...
    for rep_type in l_rep_type:
        ...
        if os.path.isfile(sf_maternal_bam) and os.path.isfile(sf_paternal_bam):#both exist
            sf_rep_bam2 = s_wfolder_rep + "/bam_list2.txt"  # save the control file list
            with open(sf_rep_bam2, "w") as fout_rep_bams2:
                fout_rep_bams2.write(sf_paternal_bam + "\n")
                fout_rep_bams2.write(sf_maternal_bam + "\n")

        if os.path.isfile(sf_case_bam) and os.path.isfile(sf_maternal_bam) and os.path.isfile(sf_paternal_bam):
            sf_rep_bam3 = s_wfolder_rep + "/bam_list3.txt"  # save the case control file list
            with open(sf_rep_bam3, "w") as fout_rep_bams3:
                fout_rep_bams3.write(s_family_id + "\t" + sf_case_bam+"\t"+sf_paternal_bam+"\t"+sf_maternal_bam+"\n")
...
####
        ...
        s_denovo_cmd=gnrt_calling_command_denovo_from_trio(iflt_clip, iflt_disc, ncores, sf_ctr_list,
                                                         iflk_len, sf_case_ctrl_list, iflag, i_rep_type)
        s_calling_cmd += s_denovo_cmd
...
def cp_compress_results(s_wfolder, l_rep_type, sample_id):
    ...
    for rep_type in l_rep_type:
        ...
        sf_rslt11 = sf_source_folder + "candidate_disc_filtered_cns_post_filtering_denovo.txt.after_parent1.txt"
        cp_file(sf_rslt11, sf_samp_folder)
        sf_rslt12 = sf_source_folder + "candidate_disc_filtered_cns_post_filtering_denovo.txt.before_final_filter"
        cp_file(sf_rslt12, sf_samp_folder)
        sf_rslt13 = sf_source_folder + "candidate_disc_filtered_cns_post_filtering_denovo.txt"
...
if __name__ == '__main__':
    ...
    sf_paternal_bam = options.paternal
    sf_maternal_bam = options.maternal
    ...
    pysam.index(sf_paternal_bam)
    pysam.index(sf_maternal_bam)
    ...
    sf_sbatch_sh=s_wfolder + s_family_id + "/run_xTea_denovo_ins.sh"
    ...
    f_purity = 1.0
    b_tumor=True
    gnrt_running_shell(s_family_id, sf_case_bam, sf_paternal_bam, sf_maternal_bam, l_rep_type, s_wfolder,
