...
#09/20/2020 Revision: for discordant pair, we check anchor mapping quality
##
                ...
                for ins_chrm in m_info:
                    for ins_pos in m_info[ins_chrm]:
                        if (ins_chrm in m_merged) and (ins_pos in m_merged[ins_chrm]):
                            ...
                            n_full_map = int(m_info[ins_chrm][ins_pos][1]) + int(m_merged[ins_chrm][ins_pos][1])
                            n_l_raw_clip = int(m_info[ins_chrm][ins_pos][2]) + int(m_merged[ins_chrm][ins_pos][2])
                            n_r_raw_clip = int(m_info[ins_chrm][ins_pos][3]) + int(m_merged[ins_chrm][ins_pos][3])
                            n_disc_pairs = int(m_info[ins_chrm][ins_pos][4]) + int(m_merged[ins_chrm][ins_pos][4])
                            ...
                            n_polyA=int(m_info[ins_chrm][ins_pos][8]) + int(m_merged[ins_chrm][ins_pos][8])
                            n_disc_chrms=int(m_info[ins_chrm][ins_pos][9]) + int(m_merged[ins_chrm][ins_pos][9])
...
####
####
    ####collect clip and discordant features of given sites
    ...
    def collect_features_one_site(self, record):#
        ...
        ins_pos = record[0][1]
        extnd = record[0][2]
        rgn_start_pos = ins_pos - extnd
        if rgn_start_pos <= 0:
            rgn_start_pos = 1
        rgn_end_pos = ins_pos + extnd
        sf_bam = record[1]
...
####
        ...
        m_clip_qname={}
        xpolyA = PolyA()
        for algnmt in samfile.fetch(chrm_in_bam, rgn_start_pos, rgn_end_pos):##fetch reads mapped to "chrm:start_pos-end_pos"
...
####
            ...
            if b_fully_mapped==True and mapq>=global_values.MINIMUM_CLIP_MAPQ:
            ...
            if l_cigar[0][0] == 4:  #left clipped
                ...
                if (abs(map_pos-ins_pos)<global_values.CLIP_EXACT_CLIP_SLACK) and (mapq>=global_values.MINIMUM_CLIP_MAPQ):
                    n_l_af_clip+=1
