...
####
# grnt calling steps
def gnrt_calling_command(iclip_c, iclip_rp, idisc_c, iflt_clip, iflt_disc, ncores, iflk_len, min_tei_len, iflag, b_wes,
    ...
    s_filter = "python ${{XTEA_PATH}}\"x_TEA_main.py\" -N --cr {0} --nd {1} -b ${{BAM_LIST}} -p ${{TMP_CNS}} " \
               ...
               "-o ${{PREFIX}}\"candidate_disc_filtered_cns.txt\" {4} {5} {6} {7}\n".format(iflt_clip, iflt_disc, iflk_len, ncores,
                                                                                                 s_purity, s_user, s_tumor, s_resume)
    if b_SVA==True:
    ...
    sf_mutation = "python ${{XTEA_PATH}}\"x_TEA_main.py\" --mutation -p ${{TMP}}\"mutation/\" -n {0} --rtype {1} " \
                  "-i ${{PREFIX}}\"candidate_disc_filtered_cns.txt.high_confident.post_filtering_with_gene_gntp.txt\" " \
                  "-b ${{BAM_LIST}} --cns ${{L1_CNS}} --ref ${{REF}} -o ${{PREFIX}}\"merged_internal_mutations.vcf\" " \
                  " --output2 ${{PREFIX}}\"merged_flanking_mutations.vcf\"\n".format(ncores, i_rep_type)

    sf_mutation_somatic="python ${{XTEA_PATH}}\"x_TEA_main.py\" --mutation -p ${{TMP}}\"mutation/\" -n {0} --rtype {1}  " \
                  "-i ${{PREFIX}}\"candidate_disc_filtered_cns.txt.high_confident.post_filtering_somatic_with_gene.txt\" " \
                  "-b ${{BAM_LIST}} --cns ${{L1_CNS}} --ref ${{REF}} -o ${{PREFIX}}\"merged_somatic_L1_internal_mutations.vcf\" " \
                  " --output2 ${{PREFIX}}\"merged_somatic_L1_flanking_mutations.vcf\"\n".format(ncores, i_rep_type)

    ####
    sf_post_filter = "python ${{XTEA_PATH}}\"x_TEA_main.py\" --postF --rtype {0} -p ${{TMP_CNS}} " \
                     "-n {1} -i ${{PREFIX}}\"candidate_disc_filtered_cns2.txt\" " \
                     "-a ${{ANNOTATION1}} {2} {3} " \
                     "-o ${{PREFIX}}\"candidate_disc_filtered_cns_post_filtering.txt\"\n".format(i_rep_type, ncores, s_tumor, s_wes)
    ...
    #s_igv="python ${{XTEA_PATH}}\"x_TEA_main.py\" --igv -i "
...
#####
    ...
    if iflag & 8192 == 8192:
        s_cmd += sf_sr_asm
    if iflag & 16384 == 16384:
        if b_tumor == True:
            s_cmd += sf_mutation_somatic
        else:
            s_cmd += sf_mutation
    return s_cmd
...
def gnrt_calling_command_non_RNA(iclip_c, iclip_rp, idisc_c, iflt_clip, iflt_disc, ncores, iflk_len, min_tei_len, iflag,
    ...
    #               "-i ${{PREFIX}}\"candidate_disc_filtered_cns.txt\" -r ${{L1_CNS}} " \
    #               "--teilen {1} -o ${{PREFIX}}\"internal_snp.vcf.gz\"\n".format(ncores, min_tei_len)
    sf_mutation = "python ${{XTEA_PATH}}\"x_TEA_main.py\" --mutation -p ${{TMP}}\"mutation/\" -n {0} " \
                  "-i ${{PREFIX}}\"candidate_disc_filtered_cns.txt.high_confident.post_filtering_with_gene_gntp.txt\" " \
                  "-b ${{BAM_LIST}} --cns ${{L1_CNS}} --ref ${{REF}} -o ${{PREFIX}}\"merged_L1_internal_mutations.vcf\" " \
                  " --output2 ${{PREFIX}}\"merged_L1_flanking_mutations.vcf\"\n".format(ncores)
    ...
    with open(sf_id) as fin_id:
        for line in fin_id:
            ...
            if os.path.exists(sf_tmp_igv) == False:
                ...
                run_cmd(cmd)
            sf_tmp_mutation = sf_folder + "/tmp/mutation"
            if os.path.exists(sf_tmp_mutation) == False:
                cmd = "mkdir {0}".format(sf_tmp_mutation)
                run_cmd(cmd)
    m_bams = {}##
    if sf_bams != "null":
...
def gnrt_running_shell(sf_ids, sf_bams, sf_10X_bams, l_rep_type, b_mosaic, b_user_par, b_force, b_tumor, f_purity, s_wfolder,
                       sf_folder_rep, sf_ref, sf_gene, sf_black_list, sf_folder_xtea, spartition, stime,
                       smemory, ncores, sf_submit_sh, sf_case_control_bam_list="null", b_wes=False, b_lsf=False,
                       b_slurm=False, b_resume=False, b_denovo=False):
    if s_wfolder[-1] != "/":#
        s_wfolder += "/"
...
####
    ...
    for sid_tmp in m_id:
        ...
        for rep_type in l_rep_type:
            ...
            iflag = options.flag #indicates which step to run

            s_calling_cmd = gnrt_calling_command(iclip_c, iclip_rp, idisc_c, iflt_clip, iflt_disc, ncores, iflk_len,
                                                 itei_len, iflag, b_wes, b_mosaic, b_user_par, b_force, b_tumor, b_resume,
                                                 f_purity, i_rep_type, sf_pub_clip)
            if rep_type==REP_TYPE_SVA:
                s_calling_cmd = gnrt_calling_command(iclip_c, iclip_rp, idisc_c, iflt_clip, iflt_disc, ncores, iflk_len,
                                                     itei_len, iflag, b_wes, b_mosaic, b_user_par, b_force, b_tumor, b_resume,
                                                     f_purity, i_rep_type, sf_pub_clip, True)
            ...
            if sf_case_control_bam_list != "null":#
                ...
                sf_tmp_cmd=gnrt_calling_command_somatic_case_control(sid_tmp, iflt_clip, iflt_disc, ncores, sf_ctr_list,
...
####sf_ori_bam in format: s_id sf_case_bam sf_control_bam1 sf_control_bam2 ...
####sf_sprt_bam in format: s_id sf_case_bam
####sf_control_bam in format: s_id sf_control_bam1 sf_control_bam2 ...
...
def parse_option():
    ...
    parser.add_option("--purity", dest="purity", type="float", default=0.45,  # by default tumor purity set to 45%
                      help="Tumor purity")
    parser.add_option("-W", "--wes",
                      action="store_true", dest="wes", default=False,
                      help="For whole exom sequencing data")  #
    ...
    parser.add_option("--cns", dest="consensus", type="string", default="none",
...
if __name__ == '__main__':
    ...
    if b_version==True:
        print(("xTea %s for short and linked reads on hg19\n" % S_VERSION))
    else:##
        sf_id = options.id
    ...
    ####
        ...
        if b_case_control == False:
            gnrt_running_shell(sf_id, sf_bams, sf_bams_10X, l_rep_type, b_mosaic, b_user_par, b_force, b_tumor, f_purity,
                               ...
                               smemory, ncores, sf_sbatch_sh, "null", b_wes, b_lsf, b_slurm, b_resume)
            ...
            gnrt_running_shell(sf_id, sf_sprt_bams, sf_bams_10X, l_rep_type, b_mosaic, b_user_par, b_force, b_tumor,
                               ...
                               spartition, stime, smemory, ncores, sf_sbatch_sh, sf_bams, b_wes, b_lsf, b_slurm, b_resume)

        else:
            # if in case-control mode, then need to separate to two steps
            # thus need to prepare the files separately
            sf_sprt_bams = sf_bams + CASE_LIST_SUFFIX
            sf_control_bams = sf_bams + CTRL_LIST_SUFFIX #not used for now
            prepare_case_control_bam(sf_bams, sf_sprt_bams, sf_control_bams)
            # first, run the jobs seperately for all the case and control samples
            gnrt_running_shell(sf_id, sf_sprt_bams, sf_bams_10X, l_rep_type, b_mosaic, b_user_par, b_force, b_tumor,
                               f_purity, s_wfolder, sf_folder_rep, sf_ref, sf_gene, sf_black_list, sf_folder_xtea, spartition,
                               stime, smemory, ncores, sf_sbatch_sh, sf_bams, b_wes, b_lsf, b_slurm, b_resume, b_denovo)
