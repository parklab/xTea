...
##Revised based on the old version to create a wrapper for somatic insertion calling from case-ctrl data

####03/07/22 the parameters are controled by purity (which is a hard code here as 0.6) ##
...
def gnrt_calling_command(iclip_c, iclip_rp, idisc_c, iflt_clip, iflt_disc, ncores, iflk_len, min_tei_len, iflag,
    ...
    sclip_step = "python ${{XTEA_PATH}}\"x_TEA_main_somatic_cloud.py\" -C -i ${{BAM_LIST}} --lc {0} --rc {1} --cr {2}  " \
    ...
    if b_SVA is True:
        sclip_step = "python ${{XTEA_PATH}}\"x_TEA_main_somatic_cloud.py\" -C --sva -i ${{BAM_LIST}} --lc {0} --rc {1} --cr {2}  " \
                     "-r ${{L1_COPY_WITH_FLANK}}  -a ${{ANNOTATION}} --cns ${{L1_CNS}} --ref ${{REF}} -p ${{TMP}} " \
                     "-o ${{PREFIX}}\"candidate_list_from_clip.txt\"  -n {3} --cp {4} {5} {6} {7} {8} {9}\n".format(iclip_c, iclip_c,
                                                                                                    iclip_rp, ncores,
                                                                                                    s_cfolder, s_purity, s_user, s_clean, s_tumor, s_resume)
    sdisc_step = "python ${{XTEA_PATH}}\"x_TEA_main_somatic_cloud.py\"  -D -i ${{PREFIX}}\"candidate_list_from_clip.txt\" --nd {0} " \
    ...
    if b_SVA is True:
        sdisc_step = "python ${{XTEA_PATH}}\"x_TEA_main_somatic_cloud.py\"  -D --sva -i ${{PREFIX}}\"candidate_list_from_clip.txt\" --nd {0} " \
                     "--ref ${{REF}} -a ${{ANNOTATION}} -b ${{BAM_LIST}} -p ${{TMP}} " \
                     "-o ${{PREFIX}}\"candidate_list_from_disc.txt\" -n {1} {2} {3} {4} {5}\n".format(idisc_c, ncores, s_purity, s_user, s_tumor, s_resume)
####
    s_filter = "python ${{XTEA_PATH}}\"x_TEA_main_somatic_cloud.py\" -N --cr {0} --nd {1} -b ${{BAM_LIST}} -p ${{TMP_CNS}} " \
    ...
    if b_SVA==True:
        s_filter = "python ${{XTEA_PATH}}\"x_TEA_main_somatic_cloud.py\" -N --sva --cr {0} --nd {1} -b ${{BAM_LIST}} -p ${{TMP_CNS}} " \
                   "--fflank ${{SF_FLANK}} --flklen {2} -n {3} -i ${{PREFIX}}\"candidate_list_from_disc.txt\" " \
                   "-r ${{L1_CNS}} --ref ${{REF}} -a ${{ANNOTATION}} " \
                   "-o ${{PREFIX}}\"candidate_disc_filtered_cns.txt\" {4} {5} {6} {7}\n".format(iflt_clip, iflt_disc, iflk_len,
                                                                                    ncores, s_purity, s_user, s_tumor, s_resume)
    sf_trsdct = "python ${{XTEA_PATH}}\"x_TEA_main_somatic_cloud.py\" --transduction --cr {0} --nd {1} -b ${{BAM_LIST}} -p ${{TMP_TNSD}} " \
...
####
    sf_sibling = "python ${{XTEA_PATH}}\"x_TEA_main_somatic_cloud.py\" --sibling --cr {0} --nd {1} -b ${{BAM_LIST}} -p ${{TMP_TNSD}} " \
    ...
    if b_SVA == True:
        sf_sibling = "python ${{XTEA_PATH}}\"x_TEA_main_somatic_cloud.py\" --sibling --cr {0} --nd {1} -b ${{BAM_LIST}} -p ${{TMP_TNSD}} " \
                     ...
                     "--rtype {4} -a ${{ANNOTATION1}} --blacklist ${{BLACK_LIST}} {5} {6} {7} " \
                     "-o ${{PREFIX}}\"candidate_sibling_transduction2.txt\"\n".format(iflt_clip, iflt_disc, iflk_len,
                                                                                      ncores, i_rep_type, s_purity,
                                                                                      s_tumor, s_resume)
    ####
    sf_post_filter = "python ${{XTEA_PATH}}\"x_TEA_main_somatic_cloud.py\" --postF --rtype {0} -p ${{TMP_CNS}} " \
                     ...
                     "-o ${{PREFIX}}\"candidate_disc_filtered_cns_post_filtering.txt\"\n".format(i_rep_type, ncores,
                                                                                                 s_tumor, s_hard_cut)
####
    sf_post_filter_hc = "python ${{XTEA_PATH}}\"x_TEA_main_somatic_cloud.py\" --postF --rtype {0} -p ${{TMP_CNS}} -n {1} " \
    ...
    # sf_sr_asm = "python ${{XTEA_PATH}}\"x_TEA_main_somatic_cloud.py\" --sr_asm -b ${{BAM_LIST}} -p ${{TMP_CNS}}\"asm\" " \
    ...
    #
    # sf_gene = "python ${{XTEA_PATH}}\"x_TEA_main_somatic_cloud.py\" --gene -a ${{GENE}} -i ${{PREFIX}}\"candidate_disc_filtered_cns.txt.high_confident.post_filtering.txt\" " \
    #           " -n {0} -o ${{PREFIX}}\"candidate_disc_filtered_cns.txt.high_confident.post_filtering_with_gene.txt\"\n".format(ncores)
    #
    # sf_cvt_gvcf = "python ${{XTEA_PATH}}\"x_TEA_main_somatic_cloud.py\" --gVCF -i ${{PREFIX}}\"candidate_disc_filtered_cns.txt.high_confident.post_filtering_with_gene.txt\" " \
...
####
####
def gnrt_calling_command_somatic_case_control(s_sample_id, iflt_clip, iflt_disc, ncores, sf_ctr_list, iflk_len,
                                              sf_case_control_bam_list, iflag, i_rep_type):
    s_cmd = ""
    s_case_control = "python ${{XTEA_PATH}}\"x_TEA_main_somatic_cloud.py\" --case_control -p ${{TMP_CNS}} -b {0} " \
    ...
    ####
    s_case_control_hc = "python ${{XTEA_PATH}}\"x_TEA_main_somatic_cloud.py\" --case_control --somatic_hc -p ${{TMP_CNS}} " \
    ...
    # ####
    # s_igv="python ${{XTEA_PATH}}\"x_TEA_main_somatic_cloud.py\" --igv --single_sample -p ${{PREFIX}}\"tmp/igv\" -b {0} " \
    #                  "-i ${{PREFIX}}\"candidate_disc_filtered_cns2.txt\" " \
    #                  " --ref \"hg19\" -e 500 " \
    #                  "-o ${{PREFIX}}\"tmp/igv/igv_screenshot.txt\"\n".format(sf_case_control_bam_list)
    # s_igv_hc = "python ${{XTEA_PATH}}\"x_TEA_main_somatic_cloud.py\" --igv --single_sample -p ${{PREFIX}}\"tmp/igv\" -b {0} " \
    ...
    #
    # #BamSnap
    # s_bamsnap = "python ${{XTEA_PATH}}\"x_TEA_main_somatic_cloud.py\" --igv --bamsnap --single_sample -p ${{PREFIX}}\"tmp/igv\" -b {0} " \
    #         "-i ${{PREFIX}}\"candidate_disc_filtered_cns2.txt\" " \
    #         " --ref ${{REF}} -e 1000 -n {1} " \
    #         "-o ${{PREFIX}}\"tmp/igv/bamsnap_screenshot.txt\"\n".format(sf_case_control_bam_list, ncores)
    # s_bamsnap_hc = "python ${{XTEA_PATH}}\"x_TEA_main_somatic_cloud.py\" --igv --bamsnap --single_sample -p ${{PREFIX}}\"tmp/igv\" -b {0} " \
    #            "-i ${{PREFIX}}\"candidate_disc_filtered_cns_high_confident_post_filtering_somatic.txt\" " \
    #            " --ref ${{REF}} -e 1000 -n {1} " \
    ...
    if iflag&1024==1024:
        s_cmd+=gnrt_calling_command_annotation_vcf_somatic(s_sample_id, ncores, i_rep_type)####
    if iflag == 17408:
        s_cmd = gnrt_calling_command_annotation_vcf_somatic(s_sample_id, ncores, i_rep_type)
    return s_cmd

def gnrt_calling_command_annotation_vcf_somatic(s_sample_id, ncores, i_rep_type):
    sf_gene = "python ${{XTEA_PATH}}\"x_TEA_main_somatic_cloud.py\" --gene -a ${{GENE}} -i ${{PREFIX}}\"candidate_disc_filtered_cns_high_confident_post_filtering_somatic.txt\" " \
              " -n {0} -o ${{PREFIX}}\"candidate_disc_filtered_cns.txt.high_confident.post_filtering_somatic_with_gene.txt\"\n".format(ncores)
    sf_cvt_gvcf = "python ${{XTEA_PATH}}\"x_TEA_main_somatic_cloud.py\" --gVCF -i ${{PREFIX}}\"candidate_disc_filtered_cns.txt.high_confident.post_filtering_somatic_with_gene.txt\" " \
                  " -o ${{PREFIX}} -b ${{BAM_LIST}} --ref ${{REF}} --rtype {0} --id {1}\n".format(i_rep_type, s_sample_id)
    s_cmd=""
    s_cmd += sf_gene
    s_cmd += sf_cvt_gvcf
    return s_cmd
...
####
    for sid in m_id:
        ...
        for sf_igv_folder in l_bamsnap_folder:
            ...
            sf_bamsnap_sh2=sf_bamsnap_sh+".focal.sh"
            l_sbatch_files.append(sf_bamsnap_sh2)#
...
def gnrt_lib_config(l_rep_type, sf_folder_rep, sf_ref, sf_gene, sf_black_list, sf_folder_xtea, sf_config_prefix):
    ...
    for s_rep_type in l_rep_type:
        if s_rep_type==REP_TYPE_L1:# for L1
            ...
            sf_anno = "ANNOTATION " + sf_folder_rep + "Pseudogene/hg19/gencode_v28_GRCh37_exon_annotation.out\n"
            sf_anno1 = "ANNOTATION1 " + sf_folder_rep + "Pseudogene/hg19/gencode_v28_GRCh37_exon_annotation.out\n"
            sf_copy_with_flank = "L1_COPY_WITH_FLANK " + sf_folder_rep + "Pseudogene/hg19/gencode_v28_GRCh37_transcript_masked.fa\n"
            sf_flank = "SF_FLANK null\n"
            sf_cns = "L1_CNS " + sf_folder_rep + "consensus/gencode_v28_GRCh37_transcript_masked.fa\n"#
            write_to_config(sf_anno, sf_anno1, sf_ref, sf_gene_anno, sf_black_list, sf_copy_with_flank, sf_flank,
...
def gnrt_running_shell(s_sample_id, sf_case_bam, sf_ctrl_bam, l_rep_type, s_wfolder,
                       sf_folder_rep, sf_ref, sf_gene, sf_black_list, sf_folder_xtea, ncores, f_purity, b_tumor, sf_submit_sh):

    b_mosaic, b_user_par, b_force, b_hard_cut, b_resume =False, False, False, False, False
    ...
    sf_folder = s_wfolder + s_sample_id  # first create folder
    if os.path.exists(sf_folder) == False:
        cmd = "mkdir {0}".format(sf_folder)
    ...
    for rep_type in l_rep_type:
        ...
        with open(sf_rep_sample_id, "w") as fout_rep_sample_id:
            fout_rep_sample_id.write(s_sample_id)
        ...
        if os.path.isfile(sf_case_bam):
            ...
            with open(sf_rep_bam, "w") as fout_rep_bams:
                fout_rep_bams.write(s_sample_id + "\t" + sf_case_bam + "\n")

        if os.path.isfile(sf_ctrl_bam):
            sf_rep_bam2 = s_wfolder_rep + "/bam_list2.txt"  # save the control file list
            with open(sf_rep_bam2, "w") as fout_rep_bams2:
                fout_rep_bams2.write(sf_ctrl_bam + "\n")

        if os.path.isfile(sf_case_bam) and os.path.isfile(sf_ctrl_bam):
            sf_rep_bam3 = s_wfolder_rep + "/bam_list3.txt"  # save the case control file list
            with open(sf_rep_bam3, "w") as fout_rep_bams3:
                fout_rep_bams3.write(s_sample_id + "\t" + sf_case_bam+"\t"+sf_ctrl_bam+"\n")##

    l_sh=[]##
    #for sid_tmp in m_id:
    sf_sample_folder=s_wfolder + s_sample_id + "/"
...
####
        ...
        s_somatic_cmd=gnrt_calling_command_somatic_case_control(s_sample_id, iflt_clip, iflt_disc, ncores, sf_ctr_list,
                                                         iflk_len, sf_case_ctrl_list, iflag, i_rep_type)
        s_calling_cmd += s_somatic_cmd
...
def cp_compress_results(s_wfolder, l_rep_type, sample_id):
    ...
    for rep_type in l_rep_type:
        ...
        sf_rslt10 = sf_source_folder + "candidate_disc_filtered_cns_high_confident_post_filtering.txt"
        cp_file(sf_rslt10, sf_samp_folder)
        sf_rslt11 = sf_source_folder + "candidate_disc_filtered_cns_post_filtering_somatic.txt"
        cp_file(sf_rslt11, sf_samp_folder)
        sf_rslt12 = sf_source_folder + "candidate_disc_filtered_cns_high_confident_post_filtering_somatic.txt"
        cp_file(sf_rslt12, sf_samp_folder)
        sf_rslt14 = sf_source_folder + "%s_%s.vcf" % (sample_id, rep_type)
        cp_file(sf_rslt14, sf_samp_folder)
        ####skip those have lots of candidates
        ####some are caused because of

        ##png files
        sf_rslt15 = sf_source_folder + "tmp/igv/"
        cp_folder(sf_rslt15, sf_samp_folder)
...
def parse_option():
    ...
    parser.add_option("--ctrl", dest="ctrl", default="null",
                      help="Input control bam file", metavar="FILE")
...
if __name__ == '__main__':
    ...
    s_sample_id = options.id #--id
    sf_case_bam = options.bam ###case bam file (not file list, but the exact bam/cram file) #--bam
    sf_ctrl_bam = options.ctrl #--ctrl
    ...
    if os.path.isfile(sf_ctrl_bam + ".bai") == False and os.path.isfile(sf_ctrl_bam + ".crai") == False:
        pysam.index(sf_ctrl_bam)
    ...
    sf_sbatch_sh=s_wfolder + s_sample_id + "/run_xTea_somatic_ins.sh"
    # first, run the jobs seperately for all the case and control samples
    print("Start command generating:")

    f_purity = options.purity
    b_tumor=True
    gnrt_running_shell(s_sample_id, sf_case_bam, sf_ctrl_bam, l_rep_type, s_wfolder,
                       sf_folder_rep, sf_ref, sf_gene, sf_black_list, sf_folder_xtea, ncores, f_purity, b_tumor, sf_sbatch_sh) ##
    ...
    cp_compress_results(s_wfolder, l_rep_type, s_sample_id)
