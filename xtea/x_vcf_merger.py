...
#Todo:
# 1) multiple threads to fetch the referece allele
#
...
####
    ...
    def gnrt_population_vcf(self, sf_sample_info, sf_rslt_list, peak_window, sf_wfolder, sf_ref, sf_out):#
        ...
        f_opened_ref=pysam.FastaFile(sf_ref)
        ####
        sf_unsorted=sf_out+".tmp_unsorted"
        sf_td_src_unsort=sf_out+".td_src.tmp_unsorted"
        sf_site_sample=sf_out+".site_sample.csv"
        sf_site_orientation_level=sf_out+".orientation_site_level.txt"#
        with open(sf_unsorted,"w") as fout, open(sf_td_src_unsort,"w") as fout_td_src, \
                open(sf_site_sample,"w") as fout_site_sample, open(sf_site_orientation_level, "w") as fout_ori_level:
            for chrm in m_peak_sites:
                for pos in m_peak_sites[chrm]:
                    l_sample_rcds=m_peak_sites[chrm][pos]##
                    s_info, m_td_src_rcd, s_pos_info, m_ori, m_level, i_end_pos= \
                        self._cvt_one_rcd_to_pop_info(chrm, pos, m_sample_info, l_samples, m_pop_cnt,
                                                      l_sample_rcds, f_opened_ref)
                    fout.write(s_info)

                    s_site_level="not_tprt_both"
                    if global_values.TWO_SIDE_TPRT_BOTH in m_level:
                        n_total_cnt=0
                        for s_tmp_level in m_level:
                            n_total_cnt+=m_level[s_tmp_level]
                        n_tprt_both=m_level[global_values.TWO_SIDE_TPRT_BOTH]
                        if float(n_tprt_both)/float(n_total_cnt)>0.9:
                            s_site_level=global_values.TWO_SIDE_TPRT_BOTH

                    if len(m_ori)>1 or len(m_ori)<=0:
                        fout_ori_level.write(chrm+"\t"+str(pos)+"\t"+"."+"\t"+s_site_level+"\n")
                    elif len(m_ori)==1:
                        l_oris=list(m_ori.keys())
                        if l_oris[0]=="-":##
                            fout_ori_level.write(
                                chrm + "\t" + str(i_end_pos) + "\t" + l_oris[0] + "\t" + s_site_level + "\n")  #
                        else:
                            fout_ori_level.write(chrm + "\t" + str(pos) + "\t" + l_oris[0] + "\t" + s_site_level + "\n")#
                    ...
                    fout_site_sample.write(chrm+"\t"+str(pos))
                    for tmp_rcd in l_sample_rcds:
                        s_tmp_sample=tmp_rcd[0]#
                        fout_site_sample.write("\t"+s_tmp_sample)
                    fout_site_sample.write("\n")
####
        f_opened_ref.close()
    ...
    def separate_reference_polymorphic_copies(self, sf_merged_vcf):
        pass
####
...
####
    ...
    def _cvt_one_rcd_to_pop_info(self, chrm, pos, m_sample_info, l_samples, m_pop_cnt, l_sample_rcd, f_opened_ref):
        ...
        m_orientation={}#this is to save the orientation of the site
        m_site_level={}#e.g. "two_side_tprt_both"#
        #m_cleavage_position={}
        for (sample_id, s_gvcf_rcd) in l_sample_rcd:
            ...
            l_tmp_fields2=l_tmp_fields[7].split(";")
            s_sv_len = l_tmp_fields2[1] #note fixed index
            if s_sv_len not in m_sv_len:
                m_sv_len[s_sv_len]=0
            m_sv_len[s_sv_len]+=1

            s_end=l_tmp_fields2[2]
            ...
            s_td_src=l_tmp_fields2[6][7:]#note fixed index
            ...
            s_orientation=l_tmp_fields2[7][-1]##
            if s_orientation not in m_orientation:
                m_orientation[s_orientation]=1
            else:
                m_orientation[s_orientation]+=1

            s_site_level=global_values.TWO_SIDE_TPRT_BOTH #"two_side_tprt_both"
            if s_site_level not in s_gvcf_rcd:
                s_site_level="not_two_side_tprt_both"

            if s_site_level not in m_site_level:
                m_site_level[s_site_level]=1
            else:
                m_site_level[s_site_level] += 1

            s_ref_rep=l_tmp_fields2[-2]
            if s_ref_rep not in m_ref_rep:
                m_ref_rep[s_ref_rep]=0
            m_ref_rep[s_ref_rep]+=1
            s_gene=l_tmp_fields2[-1]
...
########NOTE: here we only use the highest frequency one as the "max sv lenth", not the longest one!!!
        ...
        #check whether end position is larger than pos, if not, then switch them
        i_end_pos=int(s_max_end[4:])
        i_pos=int(pos)
        #print("test!!!before:",i_end_pos, i_pos) ##
        if i_end_pos<i_pos:
            i_tmp_pos=i_end_pos
            i_end_pos=i_pos
            s_max_end=s_max_end[:4]+str(i_end_pos)
            i_pos=i_tmp_pos

        s_base_seq=f_opened_ref.fetch(chrm, i_pos, i_pos+1)
        #print("test!!!after:", i_end_pos, i_pos)
        #s_position_info = "{0}\t{1}\t.\tA\t{2}\t.\tPASS\tSVTYPE={3};".format(chrm, i_pos, s_type, s_type[1:-1])
        s_position_info = "{0}\t{1}\t.\t{2}\t{3}\t.\tPASS\tSVTYPE={4};".format(chrm, i_pos, s_base_seq, s_type, s_type[1:-1])
        ...
        sinfo+="\n"
        return sinfo, m_td_src, s_position_info, m_orientation, m_site_level, i_end_pos
####
...
#-S -y 15 -i sample_info -p working_folder -x list_of_sample_folder.txt
####
if __name__ == '__main__':
    ...
    if b_single==True:#for one individual, merge all the vcf of different rep types
        ...
        pop_vcf_merger.gnrt_population_vcf(sf_sample_info, sf_rslt_list, i_peak_window, sf_wfolder, sf_ref, sf_out)
####
####