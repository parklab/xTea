...
####
    ...
    def gnrt_vcf_head(self, s_sample_id, sf_bam):
        ...
        if "SQ" not in m_header:#
            ...
            shead+=self.prepare_head_hg38(True)
        else:
            sf_ref=""
            l_chrm=[]
...
####
    ...
    def prepare_head_other(self):#
        ...
        sinfo += "##INFO=<ID=VAF,Number=A,Type=Float,Description=\"Estimated variant allele frequency in the range (0,1)\">\n"
        ...
        sinfo += "##INFO=<ID=VAF_CLIP,Number=1,Type=Integer,Description=\"Number of effective clip reads (for VAF)\">\n"
        sinfo += "##INFO=<ID=VAF_FMAP,Number=1,Type=Integer,Description=\"Number of effective fully mapped reads (for VAF)\">\n"
        sinfo += "##INFO=<ID=VAF_DISC,Number=1,Type=Integer,Description=\"Number of effective discordant pairs (for VAF)\">\n"
        sinfo += "##INFO=<ID=VAF_CONCORDNT,Number=1,Type=Integer,Description=\"Number of effective concordant pairs (for VAF)\">\n"
...
####
    ...
    def _cvt_to_vcf_rcd(self, l_fields, s_type, s_tsd, s_id, s_ref_seq):
        ...
        f_clip_AF=0
        if (n_AF_clip+n_AF_full)>0:
            f_clip_AF=float(n_AF_clip)/float(n_AF_clip+n_AF_full)
        n_raw_lclip=int(l_fields[37])
        n_raw_rclip=int(l_fields[38])
        n_AF_disc=int(l_fields[39])
        n_AF_concod=int(l_fields[40])
        f_disc_AF=0
        if (n_AF_disc+n_AF_concod)>0:
            f_disc_AF=float(n_AF_disc)/float(n_AF_disc+n_AF_concod)
        f_AF=(f_clip_AF+f_disc_AF)/2# take the average of clip-AF and disc-AF
        ...
        s_gntp="0/1" ####by default set the genotype as 0/1
        if len(l_fields)>50:
        ...
        sinfo += ("VAF="+str(f_AF)+";")#this is from clip/(clip+full-map), may not that accuracy!!!#
        ...
        sinfo += ("VAF_CLIP=" + str(n_AF_clip) +";")
        sinfo += ("VAF_FMAP=" + str(n_AF_full) +";")
        sinfo += ("VAF_DISC=" + str(n_AF_disc) +";")
        sinfo += ("VAF_CONCORDNT=" + str(n_AF_concod) +";")
...
####
    def get_VAF_from_vcf(self, sf_vcf):
        l_af=[]
        ...
        with open(sf_vcf) as fin_vcf:
            for line in fin_vcf:
                ...
                n_disc = 0
                n_concor = 1
                for s_term in l_info:
                    l_term=s_term.split("=")
                    if l_term[0]=="VAF_CLIP":
                        n_clip=int(l_term[1])
                        ...
                        n_concor=int(l_term[1])
                s_gntp=fields[-1]
                if n_clip+n_full ==0:
                    print(sf_vcf, fields[0], fields[1])
                    continue
                f_clip_af=float(n_clip)/float(n_clip+n_full)
                f_disc_af=float(n_disc)/float(n_disc+n_concor)
                l_af.append((f_clip_af, f_disc_af, s_gntp))
        return l_af

    def get_VCF_from_list_vcfs(self, sf_vcf_list, sf_out):
        with open(sf_vcf_list) as fin_list, open(sf_out,"w") as fout:
            fout.write("clip_af,disc_af,avrg,larger,gntp\n")
            for line in fin_list:
                fields=line.rstrip().split()
                sf_vcf=fields[-1]
                l_af=self.get_VAF_from_vcf(sf_vcf)
                for (f_clip_af, f_disc_af, s_gntp) in l_af:
                    f_ave=(f_clip_af+f_disc_af)/2
                    f_larger=f_clip_af
                    if f_disc_af>f_clip_af:
                        f_larger=f_disc_af
                    fout.write(str(f_clip_af)+","+str(f_disc_af)+","+str(f_ave)+","+str(f_larger)+","+s_gntp+"\n")
####
    def get_VAF_from_vcf(self, sf_vcf):
        l_af = []
        if os.path.isfile(sf_vcf) == False:
    ...
    def get_VAF_from_list_vcfs(self, sf_vcf_list, sf_out):
        with open(sf_vcf_list) as fin_list, open(sf_out, "w") as fout:
            fout.write("clip_af,disc_af,avrg,larger,gntp\n")
            for line in fin_list:
                fields = line.rstrip().split()
                ...
                for (f_clip_af, f_disc_af, s_gntp) in l_af:
                    ...
                    if f_disc_af > f_clip_af:
                        f_larger = f_disc_af
                    fout.write(str(f_clip_af) + "," + str(f_disc_af) + "," + str(f_ave) + "," + str(
                        f_larger) + "," + s_gntp + "\n")
