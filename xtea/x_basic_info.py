...
class X_BasicInfo():#
    def __init__(self, working_folder="./", n_jobs=1, sf_ref=""):
        ...
        self.UNIQ_MAP=60 #note, for rna-seq maybe a problem
####
####
...
####
    ...
    # for now, coverage is merged if there are several bams in the list
    def get_cov_is_rlth_dup_ratio_one_sample(self, sf_bam, s_sample_name, sf_ref, search_win, b_force=False):
        f_cov = 0
        rlth = 0
        mean_is = 0
        std_var = 0
        ...
        if os.path.isfile(sf_basic_info) == True and b_force == False:
            # first already exist, and is not forced to re-calc
            m_tmp_info = self.load_basic_info_with_dup_ratio_from_file(sf_basic_info)
            if len(m_tmp_info) > 0:  # load successfully
                m_info = m_tmp_info
                b_gnrt = False
        for s_bam in m_info:
            ...
            mean_is += m_info[s_bam][2]
            std_var += m_info[s_bam][3]
            f_dup_ratio+=m_info[s_bam][4]

        if b_gnrt == True:
            (f_cov, rlth, mean_is, std_var, f_dup_ratio) = self.collect_basic_info_with_dup_rate_one_sample(sf_bam, sf_ref, search_win)
            m_tmp_info = {}
            m_tmp_info[s_sample_name] = (f_cov, rlth, mean_is, std_var, f_dup_ratio)
            self.dump_basic_info_to_file(m_tmp_info, sf_basic_info)  ####
        n_sample = len(m_info)
        if n_sample == 0:
            return (0, 0, 0, 0, 0)
        return (f_cov, rlth, mean_is, std_var, f_dup_ratio)
####
...
####
    ...
    ####SC: 090921, added the duplication rate estimation
    # collect the basic information of samples
    # randomely select some sites, and calc the coverage, insert size, and read length
    def collect_basic_info_with_dup_rate_one_sample(self, sf_bam, sf_ref, search_win):
        # first randomly select some points for check the coverage
        n_sites = global_values.N_RANDOM_SITES
        l_sites = self._random_slct_site(sf_bam, sf_ref, n_sites)
        l_tmp_info = self.collect_basic_info_of_sites2(l_sites, sf_bam, search_win, search_win)
        ...
        for record in l_tmp_info:
            ...
            if dom_rlth not in m_rlth:
                ...
                m_rlth[dom_rlth] += 1
            ##
            l_dup_ratio.append(f_dup_ratio)
        ...
        # for duplication ratio
        l_dup_ratio.sort()
        n_sites=len(l_dup_ratio)
        f_ave_dup_ratio=0
        if n_sites>0:
            f_ave_dup_ratio=l_dup_ratio[int(n_sites/2)]
        return f_ave_cov, rlth, mean_is, std_var, f_ave_dup_ratio
    ...
    def load_basic_info_from_file(self, sf_basic_info):
        ...
        with open(sf_basic_info) as fin_info:
            for line in fin_info:
                ...
                dup_ratio=0
                if len(fields)>5:
                    dup_ratio=float(fields[5])
                m_basic_info[sf_bam]=(f_ave_cov, rlth, mean_is, std_var, dup_ratio)
        return m_basic_info

    # load the basic information from file
    def load_basic_info_with_dup_ratio_from_file(self, sf_basic_info):
        m_basic_info = {}
        with open(sf_basic_info) as fin_info:
            for line in fin_info:
                fields = line.split()
                ...
                mean_is = float(fields[3])
                std_var = float(fields[4])
                f_dup_ratio=float(fields[5])
                m_basic_info[sf_bam] = (f_ave_cov, rlth, mean_is, std_var, f_dup_ratio)
        return m_basic_info
    ...
    def _collect_basic_info_one_site(self, record):
        ...
        for algnmt in samfile.fetch(chrm_in_bam, start_pos, end_pos):  ##fetch reads mapped to "chrm:start_pos-end_pos"
            ...
            if algnmt.is_secondary or algnmt.is_supplementary:
                continue
            i_mapq = algnmt.mapping_quality
            if i_mapq >= self.UNIQ_MAP:
                n_uniq += 1
...
####
        ...
        #calc the duplicate rate
        f_dup_ratio=0
        if n_uniq>0:
            f_dup_ratio = float(n_dup_uniq) / float(n_uniq)
        ...
        return (chrm, insertion_pos, flcov, frcov, domint_rlth, i_acm_is, i_acm_is_squr, n_pairs, f_dup_ratio)
###
