...
####
    ...
    parser.add_option("--cov", dest="cov", type="float", default=300.0,
                      help="approximate read depth")
    parser.add_option("--iniclip", dest="iniclip", type="int", default=2,
                      help="initial minimum clip cutoff")
    parser.add_option("--dp1", dest="dp1", type="int", default=6,
                      help="minimal read depth support for internal mutation calling")
    parser.add_option("--dp2", dest="dp2", type="int", default=8,
                      help="minimal read depth support for flanking region mutation calling")
...
if __name__ == '__main__':
    ...
    if options.mit:#if this to call mitochondrial insertion, then will not filter out chrM in "intermediate_sites.py"
    ...
    if options.wes:
        global_values.set_working_on_WES()##
...
####
    elif options.mutation:#call out the internal mutations by aligning the reads, as well as the flanking mutaions
        sf_bam_list = options.bam
        sf_bam_ref = options.ref  ###reference genome, some cram file require this file to open
        s_working_folder = options.wfolder
        n_jobs = options.cores
        sf_rep_cns = options.cns  ####repeat copies/cns here
        sf_sites=options.input
        sf_merged_internal_vcf=options.output
        sf_merged_flanking_vcf = options.output2
        #n_len_cutoff=options.teilen
        i_dp_cutoff_internal=options.dp1
        i_dp_cutoff_flank = options.dp2
        i_rep_type = options.rep_type #this is used to choose the consensus sequence for internal mutation calling

        b_force=True
        rcd, basic_rcd=automatic_gnrt_parameters_case_control(sf_bam_list, sf_bam_ref, s_working_folder, n_jobs, b_force)
        rlth = basic_rcd[1]  # read length
        mean_is = basic_rcd[2]  # mean insert size
        global_values.set_insert_size(int(mean_is))
        std_var = basic_rcd[3]  # standard derivation
        ...
        extnd = max_is
        bin_size = 50000000  # block size for parallelization
        print("mean insert-size is %d, and std is %f"%(mean_is, std_var))

        xmutation=XMutation(s_working_folder)
        xmutation.set_configuration(sf_bam_list, sf_bam_ref, sf_sites, n_jobs)
        xmutation.run_mutation_calling_with_given_list(extnd, bin_size, sf_rep_cns, i_rep_type, i_dp_cutoff_internal, i_dp_cutoff_flank,
