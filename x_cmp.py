...
####
    #cmp results
    def cmp_rslts(self, i_extd, sf_unhit):
        global EXTND
        EXTND=i_extd
    ...
    #cmp results
    def cmp_rslts2(self, i_extd, sf_unhit):
        global EXTND
        EXTND=i_extd
        m_bcmk = self.load_sites(self.sf_bcmk)
        m_input = self.load_sites(self.sf_input)
...
####
####
def check_one_sample_agnst_bcmk(sf_input, sf_bcmk, sf_hc_bed, i_extd, s_type, sf_unhit):
    ...
    n_hit=rslt_cmp.cmp_rslts(i_extd, sf_unhit)#

    return n_ori_in, n_hc_in, n_hit
####
####this one output the false positives to a separate file
def check_one_sample_agnst_bcmk2(sf_input, sf_bcmk, sf_hc_bed, i_extd, s_type, sf_unhit):
    sf_hc_input = sf_input + ".focal_region.txt"
    if sf_hc_bed is None:
        sf_hc_input = sf_input
    ...
    n_hit=rslt_cmp.cmp_rslts2(i_extd, sf_unhit)#
    return n_ori_in, n_hc_in, n_hit
...
def cmp_for_list_of_rslts(sf_list, sf_bcmk, sf_hc_bed, i_extd, sf_unhit):
    m_cnt={}
    l_sample_id=[]
    m_tmp={}
    ...
    for s_sample in m_rslt_list:
        for s_rep_type in m_rslt_list[s_sample]:
            ...
            n_ori_in, n_hc_in, n_hit=check_one_sample_agnst_bcmk(sf_new_input, sf_bcmk, sf_hc_bed, i_extd, s_rep_type, sf_unhit)
...
if __name__ == '__main__':
    ...
    if b_group is True:
        m_cnt, l_sample_info, m_tp_fp_fn=cmp_for_list_of_rslts(sf_list, sf_bcmk, sf_hc_bed, i_extd, sf_unhit)
        #print m_cnt
        sf_csv=sf_unhit
        ...
        sf_new_input = sf_input + ".tmp"
        gnrt_bed_from_input(sf_input, sf_new_input)
        check_one_sample_agnst_bcmk2(sf_new_input, sf_bcmk, sf_hc_bed, i_extd, s_type, sf_unhit)
