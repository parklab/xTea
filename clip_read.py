...
def unwrap_self_collect_clip_parts_l(arg, **kwarg):
    return LRClipReadInfo.collect_clipped_parts_lr_by_chrm(*arg, **kwarg)
...
class ClipReadInfo():
    ...
    def collect_clipped_reads_with_position_by_chrm(self, record):
        ...
        sf_clip_fq = working_folder + chrm + CLIP_FQ_SUFFIX  # this is to save the clipped part for re-alignment
        f_clip_fq = open(sf_clip_fq, "w")

        samfile = pysam.AlignmentFile(sf_bam, "rb", reference_filename=self.sf_reference)
        m_clip_pos = {}
        sf_all_disc = sf_bam + ALL_DISC_SUFFIX
        ...
        for algnmt in samfile.fetch(chrm):  ##fetch reads mapped to "chrm"
            ...
            if l_cigar[0][0] == 4 or l_cigar[0][0] == 5:  # left clipped
                ...
                if l_cigar[0][0] == 4:  # soft-clip
                    ...
                    clipped_rname = query_name + SEPERATOR + mate_chrm + SEPERATOR + str(mate_pos) + SEPERATOR
                    s_tmp = "{0}{1}{2}{3}{4}{5}2".format(chrm, SEPERATOR, map_pos, SEPERATOR, FLAG_LEFT_CLIP,
                                                         SEPERATOR)
                    clipped_rname += s_tmp
                    if b_first:
                        clipped_rname = query_name + SEPERATOR + mate_chrm + SEPERATOR + str(mate_pos) + SEPERATOR
                        s_tmp = "{0}{1}{2}{3}{4}{5}1".format(chrm, SEPERATOR, map_pos, SEPERATOR, FLAG_LEFT_CLIP,
                                                             SEPERATOR)
                        clipped_rname += s_tmp
            ...
            if l_cigar[-1][0] == 4 or l_cigar[-1][0] == 5:  # right clipped
                ...
                if l_cigar[-1][0] == 4:  # soft-clip
                    clipped_rname = query_name + SEPERATOR + mate_chrm + SEPERATOR + str(mate_pos) + SEPERATOR
                    s_tmp = "{0}{1}{2}{3}{4}{5}2".format(chrm, SEPERATOR, map_pos, SEPERATOR, FLAG_RIGHT_CLIP,
                                                         SEPERATOR)
                    clipped_rname += s_tmp
                    ...
                    if b_first:
                        clipped_rname = query_name + SEPERATOR + mate_chrm + SEPERATOR + str(mate_pos) + SEPERATOR
                        s_tmp = "{0}{1}{2}{3}{4}{5}1".format(chrm, SEPERATOR, map_pos, SEPERATOR, FLAG_RIGHT_CLIP,
                                                             SEPERATOR)
                        clipped_rname += s_tmp
...
# ##########################################
    ...
    ####This function return:
    ####1. dictionary of clip position, ##in format {chrm: {map_pos: (left_cnt, right_cnt)}}
    def collect_clip_positions(self, sf_annotation, i_clip_cutoff, b_se):
        bam_info = BamInfo(self.sf_bam, self.sf_reference)
        b_with_chr = bam_info.is_chrm_contain_chr()
        samfile = pysam.AlignmentFile(self.sf_bam, "rb", reference_filename=self.sf_reference)
        references = samfile.references
        ...
        l_chrm_records = []
        for chrm in references:
            if xfilter.is_decoy_contig_chrms(chrm) == True:  ###decoy sequnces and contigs are not considered
                continue
    ...
    def collect_clipped_parts(self, sf_all_clip_fq):
        ...
        xfilter = XFilter()
        for chrm in references:
            if xfilter.is_decoy_contig_chrms(chrm) == True:  ###Here filter out those aligned to decoy sequences
    ...
    def realign_clipped_reads_to_reference(self, sf_ref, sf_all_clip_fq, sf_clip_sam):
        n_cores = self.n_jobs - 2
        if n_cores <= 0:
            n_cores = 1
        cmd = "{0} mem -T {1} -k {2} -t {3} {4} {5}  > {6}".format(BWA_PATH, BWA_REALIGN_CUTOFF,
                                                                   BWA_REALIGN_CUTOFF, n_cores, sf_ref,
                                                                   sf_all_clip_fq, sf_clip_sam)
        Popen(cmd, shell=True, stdout=PIPE).communicate()

        ##re-align the soft-clipped reads
        ##this is limited to primary alignments and non supplementary alignments

    def realign_clipped_reads_to_reference_report_all(self, sf_ref, sf_all_clip_fq, sf_clip_sam):
        n_cores = self.n_jobs - 1
        if n_cores <= 0:
            n_cores = 1
        cmd = "{0} mem -T {1} -k {2} -a -t {3} {4} {5}  > {6}".format(BWA_PATH, BWA_REALIGN_CUTOFF,
                                                                      BWA_REALIGN_CUTOFF, n_cores, sf_ref,
                                                                      sf_all_clip_fq, sf_clip_sam)
        Popen(cmd, shell=True, stdout=PIPE).communicate()

    # def is_clip_part_aligned_in_repeat_region(self, annotation_lib, algnmt):
    #    return False

    def decrease_clip_freq(self, chrm, mpos, sflag, m_clip_freq):
        if sflag == FLAG_LEFT_CLIP:
    ...
    def correct_num_of_clip_by_realignment(self, sf_ref, sf_annotation, sf_all_clip_fq, m_clip_freq):
        ...
        sf_clip_bam = self.working_folder + "all" + CLIP_BAM_SUFFIX
        self.realign_clipped_reads_to_reference(sf_ref, sf_all_clip_fq, sf_clip_bam)
        ...
        with open(sf_clip_bam) as fin_sam:
            for line in fin_sam:
                ...
                if b_within_rep == False:
                    b_fake = True
                if b_fake == True:
                    qname_fields = qname.split(SEPERATOR)
                    # chrm, map_pos, FLAG_LEFT_CLIP, first_read
        ...
        # def integrate_clip_info(self, sf_clip_fq, sf_ref):
        #     m_clip_pos_freq=self.collect_clipped_reads(sf_clip_fq)
        #     self.realign_clipped_reads_to_reference()
        ####
    ...
    def run_cnt_clip_part_aligned_to_rep_by_chrm_sort_version(self, record):
        ...
        with open(sf_re_align_ori_clip_pos_list_sorted) as fin_ori_sorted, open(sf_out_clip_pos, "w") as fout_clip_pos:
            ...
            for line in fin_ori_sorted:
                ...
                if b_first == True:
                    b_first = False
                if fields[1] == FLAG_LEFT_CLIP:
                    cnt_left_tmp += 1
                elif fields[1] == FLAG_RIGHT_CLIP:
                    cnt_right_tmp += 1
                s_last = sinfo
    ...
    def run_cnt_clip_part_aligned_to_rep_by_chrm(self, record):
        ...
        for algnmt in samfile.fetch():  ##fetch reads mapped to "chrm"
            ...
            b_left = True
            if qname_fields[-2] == FLAG_RIGHT_CLIP:
                b_left = False
    ...
    ####
    def merge_clip_positions(self, sf_out):
        samfile = pysam.AlignmentFile(self.sf_bam, "rb", reference_filename=self.sf_reference)
        references = samfile.references
        ...
        with open(sf_out, "w") as fout_clip_pos:
            for chrm in references:
                ...
                m_realign_pos = {}
                sf_chrm_re_align_clip_pos = self.working_folder + chrm + CLIP_RE_ALIGN_POS_SUFFIX
...
class LRClipReadInfo():
    ...
    def collect_clipped_parts_lr_by_chrm(self, record):
        ...
        sf_out = working_folder + chrm + LCLIP_FA_SUFFIX
        with open(sf_out, "w") as fout_clip:
            for algnmt in bamfile.fetch(chrm):
                ...
                if l_cigar[0][0] == 4:  # left clipped
                    ...
                    shead=chrm+SEPERATOR+str(hit_site)+SEPERATOR+query_name+"L"
                ...
                if l_cigar[-1][0] == 4:  # right clipped
                    ...
                    shead = chrm + SEPERATOR + str(hit_site) + SEPERATOR + query_name + "R"
                    fout_clip.write(">" + shead + "\n")
                    fout_clip.write(clip_seq + "\n")
        bamfile.close()
####
####
    ...
    def collect_clipped_parts(self, sf_sites, islack, sf_all_clip_fa):
        ...
        with open(sf_all_clip_fa, "w") as fout_all:
            for chrm in references:
                sf_clip_fa = self.working_folder + chrm + LCLIP_FA_SUFFIX
